...
function displayWord(sel) {
    ...
    if (index <= 10) {
        ...
        //change this
        window.location.href = 'continueTest.4.html';
    }
    index++;
...
function writeToDB(currWord, selColor, randColor) {
    ...
    if (currWord != "") {
        ...
        fireBaseRef.child("Participants Data").child(uKey).child("WordTest 3").child(currWord.toUpperCase()).child("Displayed In").set(selColor);
        fireBaseRef.child("Participants Data").child(uKey).child("WordTest 3").child(currWord.toUpperCase()).child("Selected").set(randColor);
        fireBaseRef.child("Participants Data").child(uKey).child("WordTest 3").child(currWord.toUpperCase()).child("Status").set(status);
        fireBaseRef.child("Participants Data").child(uKey).child("WordTest 3").child(currWord.toUpperCase()).child("Time").set(time + " ms");
    }
