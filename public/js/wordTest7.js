...
var spanish3 = ["barril", "mesa", "belleza", "acuerdo", "recompensa", "suicidio", "dolor", "eyacular", "verga", "chocha", ];
var english3 = ["barrel", "table", "beauty", "agreement", "reward", "suicide", "pain", "ejaculate", "dick", "pussy"];
...
function displayWord(sel) {
    ...
    if (index <= 10) {
        ...
        //change this
        window.location.href = 'continueTest.8.html';
    }
    index++;
...
function writeToDB(currWord, selColor, randColor) {
    ...
    if (currWord != "") {
        ...
        fireBaseRef.child("Participants Data").child(uKey).child("WordTest 7").child(currWord.toUpperCase()).child("Displayed In").set(selColor);
        fireBaseRef.child("Participants Data").child(uKey).child("WordTest 7").child(currWord.toUpperCase()).child("Selected").set(randColor);
        fireBaseRef.child("Participants Data").child(uKey).child("WordTest 7").child(currWord.toUpperCase()).child("Status").set(status);
        fireBaseRef.child("Participants Data").child(uKey).child("WordTest 7").child(currWord.toUpperCase()).child("Time").set(time + "ms");
    }
